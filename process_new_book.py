...
"""
    ...
    def generate_book_summary(
        ...
        chapter_previews: dict[int, dict]
    ) -> dict:
        """
        Generate a book-level summary with overall thesis and chapter breakdown.

        Args:
            book_title: Title of the book
            book_author: Author of the book
            chapters: List of all chapters
            chapter_previews: Dict of chapter number -> preview data

        Returns:
            Dict with 'book_overview', 'key_themes', 'chapter_map'
        """
        client = self._get_client()
        ...
        prompt = f"""You are creating a book overview to help a reader decide whether to read this book.
...
Generate a comprehensive book overview with:
...
3. CHAPTER MAP (brief, one line per chapter)
   A quick reference showing what each chapter covers - like an enhanced table of contents.
...
{{
  ...
  "chapter_map": [
    {{"number": 1, "title": "...", "summary": "One line description"}},
    {{"number": 2, "title": "...", "summary": "One line description"}}
  ]
}}
```
"""
    ...
    def _create_book_overview_html(self, book_summary: dict, book_title: str) -> str:
        ...
        themes = book_summary.get('key_themes', [])
        chapter_map = book_summary.get('chapter_map', [])

        themes_html = '\n'.join(f'<li>{theme}</li>' for theme in themes) if themes else ''

        chapters_html = ''
        if chapter_map:
            chapters_html = '<h3 style="margin-top: 1.5em;">Chapter Guide</h3>\n<ul style="padding-left: 1.5em;">\n'
            for ch in chapter_map:
                chapters_html += f'<li><strong>{ch.get("title", "Chapter")}</strong>: {ch.get("summary", "")}</li>\n'
            chapters_html += '</ul>'

        return f'''
<div class="jarvis-book-overview" style="padding: 1.5em; margin: 1em 0;">
  <h2 style="margin: 0 0 1em 0; border-bottom: 2px solid #333; padding-bottom: 0.5em;">📚 Book Overview</h2>

  <p style="line-height: 1.8; margin-bottom: 1.5em;">{overview}</p>
  ...
  <p style="margin-top: 2em; font-style: italic; color: #666; font-size: 0.9em;">
    This overview was generated by Jarvis to help you decide what to read.
  </p>
</div>
'''
    ...
    def build_enhanced_epub(
        ...
        """
        Build enhanced EPUB with injected preview/questions and book overview.
        ...
        Args:
            ...
            book_summary: Optional book-level summary to inject at start
            book_title: Book title for the overview
        ...
        try:
            ...
            # Inject book overview into the first content chapter
            if book_summary and chapters:
                first_content_chapter = None
                for ch in chapters:
                    if ch.number in enhancements:
                        first_content_chapter = ch
                        break

                if first_content_chapter:
                    try:
                        full_path = parser._resolve_path(first_content_chapter.epub_href)
                        ...
                        if file_path.exists():
                            ...
                            if body:
                                overview_html = self._create_book_overview_html(book_summary, book_title or "")
                                overview_soup = BeautifulSoup(overview_html, 'html.parser')
                                body.insert(0, overview_soup)
                                file_path.write_text(str(soup), encoding='utf-8')
                                logger.info(f"  Injected book overview into first chapter")
                    except Exception as e:
                        logger.warning(f"Could not inject book overview: {e}")
...
class BookProcessingPipeline:
    ...
    def process(
        ...
        # Step 4b: Generate book-level summary
        book_summary = None
        if self.anthropic_api_key and enhancements:
            step_start = time.time()
            logger.info("[4b/10] Generating book summary...")
            try:
                enhancer = EPUBLearningEnhancer(self.anthropic_api_key, reader_context)
                book_summary = enhancer.generate_book_summary(
                    metadata.title or "Unknown",
                    metadata.author,
                    chapters,
                    enhancements
                )
                logger.info(f"  Generated overview: {book_summary.get('book_overview', '')[:80]}...")
                logger.info(f"  Key themes: {len(book_summary.get('key_themes', []))}")
                results['book_summary'] = book_summary
            except Exception as e:
        ...
        if not preview:
            ...
            if book_summary:
                ...
                # Build a formatted summary for Notion sync
                ...
                if chapter_map:
                    summary_parts.append("\n\n**Chapter Guide:**")
                    for ch in chapter_map:
                        summary_parts.append(f"- **{ch.get('title', 'Chapter')}**: {ch.get('summary', '')}")

                update_data['summary'] = '\n'.join(summary_parts)
                logger.info(f"  Storing book summary ({len(update_data['summary'])} chars)")

            self.supabase.table('books').update(update_data).eq('id', book['id']).execute()
